...
import com.fcfb.arceus.util.ml.XGBoostModelLoader
...
class WinProbabilityServiceTest {
    ...
    private val winProbabilityService = WinProbabilityService(teamService, xgboostPredictor)

    @Test
    fun `should load XGBoost model successfully`() {
        // Given
        val mockModelLoader = mockk<XGBoostModelLoader>(relaxed = true)
        every { xgboostPredictor.modelLoader } returns mockModelLoader
        every { mockModelLoader.getNumTrees() } returns 278
        every { mockModelLoader.getNumFeatures() } returns 9
        every { mockModelLoader.getFeatureNames() } returns listOf("score_diff", "time_remaining", "down", "yards_to_go", "ball_location", "elo_diff", "timeout_diff", "field_position", "quarter")
        every { mockModelLoader.getBaseScore() } returns 0.5
        ...
        assertEquals("XGBoost", result["model_type"])
        assertEquals(278, result["num_trees"])
    ...
    fun `should create feature array correctly`() {
        ...
        every { xgboostPredictor.createFeatureArray(any(), any(), any(), any(), any(), any(), any(), any(), any()) } returns doubleArrayOf(1.0, 10.0, 50.0, 7.0, 900.0, 420.0, 1.0, 0.0, 200.0)
