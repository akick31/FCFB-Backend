...
class LeagueStatsController(
    ...
    fun getLeagueStatsBySubdivision(@RequestParam subdivision: String): ResponseEntity<List<LeagueStats>> {
        val subdivisionEnum = try {
            Subdivision.valueOf(subdivision.uppercase())
        } catch (e: IllegalArgumentException) {
            return ResponseEntity.badRequest().build()
     ...
     */
    @GetMapping("/season")
    fun getLeagueStatsBySeason(@RequestParam seasonNumber: Int): ResponseEntity<List<LeagueStats>> {
